...
def get_noun_words_from_sentence(sentence, mecab, stopword_list=[]):
    return [
        ...
        if len(x.split("\t")) > 1
        and "名詞" in x.split("\t")[3]
        and x.split("\t")[0] not in stopword_list
    ]
...
def make_jaccard_coef_data(combination_sentences):
    ...
    word_associates = pd.DataFrame(
        word_associates, columns=["word1", "word2", "intersection_count"]
    )
    ...
    word_associates = (
        ...
        .merge(word_count.rename(columns={"word": "word2"}), on="word2", how="left")
        .rename(columns={"count": "count2"})
        .assign(union_count=lambda x: x.count1 + x.count2 - x.intersection_count)
        .assign(jaccard_coef=lambda x: x.intersection_count / x.union_count)
        .sort_values(["jaccard_coef", "intersection_count"], ascending=[False, False])
    )
...
# 日本語が含まれたタグを削除
all_tags = [
    tag for tag in all_tags if isinstance(tag, str) and re.match(r"^[a-zA-Z\s]+$", tag)
]
# tags = ['Python', 'javascript']

for tag in all_tags:
    lines = ""
    with open(f"data/text/qiita_{tag}.txt", "r", encoding="utf-8") as file:
    ...
    noun_sentences = list(
        filter(lambda x: len(x) > 1 and "見出し" not in x, noun_sentences)
    )

    combination_sentences = []
    combination_sentences = [
        list(itertools.combinations(words, 2)) for words in noun_sentences
    ]
    combination_sentences = [
        [tuple(sorted(combi)) for combi in combinations]
    ...
    # 初期化
    jaccard_coef_data = pd.DataFrame(
        columns=[
            "word1",
            "word2",
            "intersection_count",
    ...
    jaccard_coef_data = jaccard_coef_data.drop(
        ["intersection_count", "count1", "count2", "union_count"], axis=1
    )
    ...
    output_jaccard_coef_data = pd.concat(
        [
            output_jaccard_coef_data,
            jaccard_coef_data[jaccard_coef_data["word1"].isin(all_tags)],
        ]
    )
    # output_jaccard_coef_data2 = pd.concat([output_jaccard_coef_data2, output_jaccard_coef_data[output_jaccard_coef_data['word2'].isin(all_tags)]])
    # word1とword2が同じものを消す
    output_jaccard_coef_data2 = output_jaccard_coef_data2[
        output_jaccard_coef_data2["word1"] != output_jaccard_coef_data2["word2"]
    ]
    # print(output_jaccard_coef_data)
    # output_jaccard_coef_dataをcsvファイルとして出力
    output_jaccard_coef_data.to_csv(
        f"result/en_only/jaccard_coef_data_{tag}.csv", index=False
    )
